...
int hasonlytwomatchingadjecentdigits(int n);
...
int main(int argc, char **argv) {
	...
	for (int i = start + 1; i < end; i++) {
		if (issixdigits(i) && hasonlytwomatchingadjecentdigits(i) && digitsneverdecrease(i)) {
			count++;
		}
	}
...
int hasonlytwomatchingadjecentdigits(int n) {
	int prevdigit;
	while (n) {
		if ((n % 10 == (n / 10) % 10) && (n % 10 != (n / 100) % 10) && n % 10 != prevdigit) {
			return 1;
		}
		prevdigit = n % 10;
		n /= 10;
